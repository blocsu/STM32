...
void DAC_ini(void) 
{
	...
	GPIO_struct_DAC.GPIO_Pin   = GPIO_Pin_5; //конфигурируем структуру с параметрами на ножке PA4
  ...
  DAC_InitStruct.DAC_OutputBuffer                 = DAC_OutputBuffer_Enable; //буфеп использовать необходимо чтобу DAC работал корректно
	...
	DAC_Init(DAC_Channel_2, &DAC_InitStruct); //функци€ инициализирующа€ DAC
	DAC_Cmd(DAC_Channel_2, ENABLE); //включаем DAC	
}
